            ...
            <div class="collapse navbar-collapse" id="navbarTogglerDemo03">
                <ul class="navbar-nav mr-auto mt-2 mt-lg-0">
                    ...
                    <li class="nav-item active">
                        <a class="nav-link" href="./user_draft.jsp">Draft <span class="sr-only">(current)</span></a>
                    </li>
                    <li class="nav-item">
            ...
            <div class="main-headings">
                ...
                <table id="mail-table" class="table table-hover common-table">
                    <%
                        ...
                        String page_name = "draft";
                        try {
                            Class.forName(db_driver);
                            Connection con = DriverManager.getConnection(db_url, db_username, db_password);
                            db_sql = "SELECT users_xmails_unique_id, users_xmails_user_subject_xmail, "
                                    + "users_xmails_user_sent_date FROM users_xmails WHERE "
                                    + "users_xmails_sender_users_unique_id=? AND users_xmails_trash_status=0 AND users_xmails_save_draft=1 ORDER BY users_xmails_user_sent_date DESC";
                            PreparedStatement ps = con.prepareStatement(db_sql);
                            ps.setString(1, users_unique_id);
                            ResultSet rs = ps.executeQuery();
                            while(rs.next()) {
                    ...
                    %>
                                <tr>
                                    ...
                                    <td><i><%=Sent_Date%></i></td>
                                    <td><a class="fa-icon" href="./user_composer.jsp?xmail_user=<%=rs.getString("users_xmails_unique_id")%>&xmail_page=<%=page_name%>" title="Open"><i class="fa fa-eye"></i></a></td>
